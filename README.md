...
The input of the system is defined by the *Coordinator* which input the coordinates of all robots to the *Planner*. *Planner* output a high-level plan which is a sequence of action primitives. Then the system output which is path or trajectory generated by *Actuator*. And then sent to the robot by *Mediator Server*. 
